...
public  class QueryRunnable implements Runnable{
	...
	private int endpage;
	private Exchanger<ThreeTuple<Integer,QueryBean,List<TaobaoProduct>>> exchanger;
	...
	public QueryRunnable(QueryBean queryBean, int startpage, int endpage,Exchanger<ThreeTuple<Integer,QueryBean,List<TaobaoProduct>>> exchanger,CountDownLatch downLatch) {
		super();
		...
		try {
			for(int i=startpage;i<endpage;i++){
				...
				if(result != null){
					...
					logger.error(queryBean.getKey()+"==>下载第"+startpage+ "页异常!!!!!");
				}
			}
			exchanger.exchange(ThreeTupleUtil.tuple(1,queryBean, productList));// 与主线程交换信息
